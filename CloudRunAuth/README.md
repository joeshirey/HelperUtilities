...
The script deploys a temporary, public `hello-app` container to Cloud Run and guides you through running `setup-cloud-run-oauth.sh` against it. 

When `setup-cloud-run-oauth.sh` prompts you during the validation run, enter the exact values displayed in the script's summary banner:

- **GCP Project ID:** Enter your project ID.
- **Cloud Run service name:** Enter the temporary `oauth-test-...` service name generated by the harness.
- **Cloud Run region:** Enter your chosen region (e.g., `us-central1`).
- **Allowed email domains:** Enter your test domain (e.g., `example.com`).
- **Startup probe path:** Enter `/`. (The sample `hello-app` serves a simple web page at the root path).
- **CMD override:** Enter `n`. (The sample `hello-app` automatically detects and respects the `PORT` environment variable, so no manual command override is needed).

Once the setup script finishes, the harness automatically verifies that unauthenticated requests return an HTTP `302` redirect to Google OAuth and that `allUsers` has invoker permissions. Finally, it cleans up the temporary Cloud Run service and generated YAML files.
